...
export default function LogsModal({ isOpen, onClose, activeLoad }: LogsModalProps) {
  ...
  const bootLogs = [
    'Initializing AetherOS Core (v4.1.14-Release)...',
    'Starting node supervisor services... [OK]',
    'Connecting to main gateway broker at ssl://mqtt.aetheros.io:8883... [CONNECTED]',
    'Syncing office layout matrices... done.',
    'Restoring cached hardware configurations from localStorage... [SUCCESS]',
  ...
  const handleCommandSubmit = (e: React.FormEvent) => {
    ...
    setTerminalLines((prev) => [...prev, `aetheros@hq-alpha:~$ ${inputValue}`, response]);
    setInputValue('');
  };
        ...
        <div className="fixed inset-0 z-100 flex items-center justify-center p-4">
          ...
          >
            ...
            <div className="flex justify-between items-center mb-3 pb-2 border-b border-outline-variant/30">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-mono text-xs text-on-surface-variant flex items-center gap-1.5">
                  <Terminal className="w-4 h-4 text-secondary" /> aetheros@hq-alpha:~
                </h3>
              </div>
              <button
                onClick={onClose}
            ...
            <form onSubmit={handleCommandSubmit} className="mt-3 flex items-center gap-2 bg-[#07090e] border border-outline-variant/20 rounded-lg px-3 py-2">
              <span className="font-mono text-xs text-primary shrink-0">aetheros@hq-alpha:~$</span>
              <input
                type="text"
                value={inputValue}
